    ...
    I: ValueInput<'src, Span = Location<Offset>, Token = Token>,
{
    ...
    let comment = WhiteSpace::new()
        .with_limit_comment()
        .with_progress()
        .parser();
    let ws = WhiteSpace::new().with_no_comment().with_progress().parser();
    ...
    let one_block = choice((ws, message, enum_parser, comment)).recover_with(via_parser(recovery));
...
where
    I: ValueInput<'src, Span = Location<Offset>, Token = Token>,
{
    let bind_comment = WhiteSpace::new().with_bind_comment().parser().or_not();
    let ws = WhiteSpace::new().parser();
    ...
    I: ValueInput<'src, Span = Location<Offset>, Token = Token>,
{
    ...
    let inside = choice((ws, field, comment))
        .repeated()
        .collect::<Vec<_>>()
        .map(Vec::flatten);
...
{
    let bind_comment = WhiteSpace::new().with_bind_comment().parser().or_not();
    let ws = WhiteSpace::new().with_no_new_line().parser();
    ...
    I: ValueInput<'src, Span = Location<Offset>, Token = Token>,
{
    ...
    let field_init_last = field_init
        .clone()
        .map_tree(TreeKind::ConstructedValueField)
        .map_child();

    let field_init_not_last = field_init
        .then(comma)
        .map_tree(TreeKind::ConstructedValueField)
        .map_child();

    let field_r = field_init_not_last
        .repeated()
        .collect::<Vec<_>>()
        .then(field_init_last);

    let inner = field_r
        .map(|a| (Some(a), None))
        .or(ws.clone().map(|a| (None, Some(a))));
    ...
    I: ValueInput<'src, Span = Location<Offset>, Token = Token>,
{
    ...
    let inside = choice((ws, branch, comment))
        .repeated()
        .collect::<Vec<_>>()
        .map(Vec::flatten);
    let branched = lbrace
    ...
    I: ValueInput<'src, Span = Location<Offset>, Token = Token>,
{
    ...
    let inside = choice((ws, constructor, comment))
        .repeated()
        .collect::<Vec<_>>()
        .map(Vec::flatten);
    ...
    I: ValueInput<'src, Span = Location<Offset>, Token = Token>,
{
    ...
    let constructor_with_body = bind_comment
        .clone()
    ...
    let constructor_no_body = bind_comment
        .or_not()
        .then(type_indent)
        .map_tree(TreeKind::Constructor);
