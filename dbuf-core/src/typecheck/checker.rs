...
use crate::ast::elaborated::{self, Module, Type, Constructor, ConstructorNames};
...
use super::{
    ...
    context::Context,
    interning::{InternedString, StringInterner, InternedModule, InternedExpression, InternedPattern},
...
impl TypeChecker {
    ...
    fn execute_task<Loc>(
        ...
        match task {
            ...
            CheckerTask::Branch { type_name, branch_index } => {
    ...
    fn check_constructor<Loc>(
        ...
        let (constructor_def, parent_type) = self.find_constructor_in_module(constructor_name, module)
        ...
        let parent_type_info = self.types.get(&parent_type)
        ...
        for field_def in constructor_def {
            ...
            self.context.bind_variable(field_def.name, field_type.clone())?;
        ...
        self.constructors.insert(constructor_name, elaborated_constructor);
    ...
    ) -> Option<(&'a Vec<parsed::definition::Definition<Loc, InternedString, parsed::Expression<Loc, InternedString>>>, InternedString)> {
    ...
    fn build_elaborated_module(&self) -> Result<Module<InternedString>, TypeCheckError> {
        ...
        let constructors: std::collections::BTreeMap<_, _> = self.constructors.clone().into_iter().collect();
